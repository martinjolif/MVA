...
checkpoint = torch.load('model_deepsets.pth.tar', weights_only=True)
...
checkpoint = torch.load('model_lstm.pth.tar', weights_only=True)
...
for i in range(len(cards)):
    ...
    for j in range(0, n_samples_per_card, batch_size):
        ...
        y_batch = torch.FloatTensor(y_test[i][j:min(j+batch_size, n_samples_per_card)])
